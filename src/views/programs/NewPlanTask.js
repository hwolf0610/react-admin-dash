...
class NewPlantask extends Component {
    ...
    componentDidMount = () => {
        ...
        let { addCustomArray, addCircuitArray, loaderFlag, BackendCardLibraryData, BackendPlanVersionDayTaskData, ExerciseCardSearchArray, ExerciseSubAllCardsDataArray, ExerciseTopLevelSelectOptionsArray, DayTaskTitle, DayTaskDescription } = this.state
                                ...
                                if (res.data.status == 200) {
                                    ...
                                    if (userTotalData[0].versionDayTaskCard.length > 0) {
                                        ...
                                        userTotalData[0].versionDayTaskCard.map((item, idx) => {
                                            if (item.flag == "circuit") {
                                                swapBody = {}
                                                swapBody.flag = "circuit"
                                                swapBody.id = item.id
                                                swapBody.Cycles = item.Cycles
                                                swapBody.number = 0
                                                swapBody.exeerciseCards = []
                                                swapArray.push(...item.exeerciseCards)
                                            } else {
                                        ...
                                        swapBody.exeerciseCards = swapArray
                                        addCircuitArray = this.mergeCircuitArray()
                                        addCircuitArray.push(swapBody)
                                        this.setCircuitArray(addCircuitArray)

                                    } else {
                                        console.log("userTotalData_item___:", userTotalData[0], "+++")
                                    }
                                    loaderFlag = false
                                    this.setState({ addCustomArray, addCircuitArray, loaderFlag, BackendCardLibraryData, BackendPlanVersionDayTaskData, ExerciseCardSearchArray, ExerciseSubAllCardsDataArray, ExerciseTopLevelSelectOptionsArray, DayTaskTitle, DayTaskDescription })
                                } else {
                    ...
                    onClick: () => {
                        ...
                        if (localStorage.getItem("editPlanVersionDayTaskFlag") != "") {
                            if (DayTaskTitle != "" && DayTaskDescription != "") {
                                ...
                                // formData.set("editPlanVersionID", localStorage.getItem("planVersionUUID"));
                                // formData.set("planVersionDayTaskDataID", localStorage.getItem("editPlanVersionDayTaskFlag"));
                                // formData.set("versionDay", localStorage.getItem("PlanVersionTaskDay"));
                                // formData.set("taskTitle", DayTaskTitle);
                                // formData.set("taskDescription", DayTaskDescription);
                                // formData.append("versionDayTaskCard", AllData);
    ...
    onAddCircuit = () => {
        ...
        newBodySub.title = "";
        newBodySub.cardExerciseID = "";
        newBodySub.cardUUID = "";
        newBodySub.description = "";
        newBodySub.file = null;
        newBodySub.fileName = "";
        newBodySub.thumbnailFileName = "";
        newBodySub.Reps = "Seconds";
        newBodySub.Sets = "";
        newBodySub.RepsCount = "";
        newBodySub.RestTime = "";
        newBodySub.AutoPlay = "checked";
        newBodySub.AutoPlayShowFlag = true;
    ...
    onAddCircuitSubExercise = (addItem, flag) => {
        let newBody = {}
        switch (flag) {
            case "exercise":
                newBody.flag = flag;
                newBody.id = flag + "_circuit" + date.getFullYear() + date.getMonth() + date.getDate() + date.getHours() + Math.random().toString().replace("0.", "")
                newBody.title = "";
                newBody.cardExerciseID = "";
                newBody.cardUUID = "";
                newBody.description = "";
                newBody.Reps = "Seconds";
                newBody.RepsCount = "10";
                newBody.RestTime = "5";
                newBody.AutoPlay = "checked";
                newBody.AutoPlayShowFlag = true;
                break;
            case "video":
                newBody.flag = flag;
                newBody.id = flag + "_circuit" + date.getFullYear() + date.getMonth() + date.getDate() + date.getHours() + Math.random().toString().replace("0.", "")
    ...
    ////////////////////////// Circuit Exercise value edit part -- start -- /////////////
    ...
    onChangeCardFile = (TopItems, Subitem, event) => {
        ...
        addCircuitArray = this.mergeCircuitArray()
        var dIndex = -1, videoFileName = null;
        addCircuitArray.map((item, idx) => {
        ...
        if (event.target.files[0]) {
            ...
            formDataVideo.set("time", date.getFullYear() + "-" + date.getMonth() + "-" + date.getDate() + "-" + date.getDay() + "-" + date.getHours() + "-" + date.getMinutes());
            formDataVideo.append("videoFile", event.target.files[0]);

            let urlVideo = Config.host + Config.Plan.add_VideoExercise_file;
            ApiService.apiCall('post', urlVideo, formDataVideo, (res) => {
                try {
                    if (res.data.status == 200) {
                        console.log("success:", res.data.data)

                        videoFileName = res.data.data.title
                        console.log("title:", videoFileName)
    ...
    onChangeCardImageFile = (TopItems, Subitem, event) => {
        let { addCircuitArray } = this.state
        this.setState({ loaderFlag: true });
            ...
            ApiService.apiCall('post', urlVideo, formDataVideo, (res) => {
                try {
                    if (res.data.status == 200) {
                        ...
                        addCircuitArray[dIndex].exeerciseCards.map((item, idx) => {
                            if (item.id == Subitem.id) {
                                item.thumbnailFileName = videoFileName
                                this.setState({ loaderFlag: false });
                                // item.file = event.target.files[0]                                
                            }
                        })
    ...
    onChangesCardRepsDropItem = (TopItems, Subitem, event) => {
        ...
        addCircuitArray[dIndex].exeerciseCards.map((item, idx) => {
            if (item.id == Subitem.id) {
                ...
                if (event.target.value == "Rep Count") {
                    item.AutoPlayShowFlag = false
                    item.AutoPlay = "false"
                } else {
                    item.AutoPlayShowFlag = true
                    item.AutoPlay = "checked"
                }
            }
        })
        this.setCircuitArray(addCircuitArray)
    ...
    ////////////////////////// Circuit Exercise value edit part -- End -- /////////////


    ////////////////////////////// Divided exercise card value edit part -- start ----/////////////////////////////////////////
    ...
    onChangeDividesCardImageFile = (Subitem, event) => {
        let { addCustomArray, loaderFlag } = this.state
        this.setState({ loaderFlag: true });
    ...
    ////////////////////////////// Divided exercise card value edit part -- end ----/////////////////////////////////////////




    ////Modal Control part --start --- //////
    ...
    onSelectExerciseCardImage = (seleteItem) => {
        let { ExerciseCardSearchArray, BackendCardLibraryData } = this.state
        ExerciseCardSearchArray.map((item, key) => {
                    ...
                    if (localStorage.getItem("ExerciseCardTitleFlag") == "two") {
                        ...
                        addCircuitArray[dIndex].exeerciseCards.map((Subitem, idx) => {
                            if (Subitem.id == localStorage.getItem("ExerciseCircuitSubItemTitleUUID")) {
                                ...
                                Subitem.cardExerciseID = item.id
                                console.log("UUID1", item.id)

                                BackendCardLibraryData.map(cardTop => {
                                    cardTop.exercisesData.map(cardSub => {
                                        if (cardSub.id == item.id) {
                                            Subitem.cardUUID = cardTop._id
                                            console.log("card_UUID1", cardTop._id)
                                        }
                                    })
                                })
                        ...
                        addCustomArray.map((Subitem, subIndex) => {
                            if (Subitem.id == localStorage.getItem("ExerciseCircuitTopItemsTitleUUID")) {
                                Subitem.title = item.exerciseName
                                Subitem.cardExerciseID = item.id
                                console.log("UUID2", item.id)

                                BackendCardLibraryData.map(cardTop => {
                                    cardTop.exercisesData.map(cardSub => {
                                        if (cardSub.id == item.id) {
                                            Subitem.cardUUID = cardTop._id
                                            console.log("card_UUID2", cardTop._id)
                                        }
                                    })
                                })
                        ...
                        return (
                            <div key={TopIndex}>
                                <CListGroup>
                                                ...
                                                <CListGroupItem key={subIndex}>
                                                            ...
                                                            <div style={{ float: 'right' }}>
                                                                <div style={{ display: 'inline-block' }}>
                                                                    <CSelect custom name="select" id="select" value={item.Reps} onChange={this.onChangesCardRepsDropItem.bind(this, TopItems, item)}>
                                                                        ...
                                                                        <option value="Rep Count">Reps</option>
                                                                    </CSelect>
                                                                </div>
                                                                ...
                                                                <div style={{ display: 'inline-block', marginLeft: '15px' }}> {item.Reps}</div>
                                                                <div style={{ display: 'inline-block', marginLeft: '15px' }}>
                                                                    <CInput type="email" id="date-input" name="date-input" placeholder="10" style={{ width: "70px" }} value={item.RepsCount} onChange={this.onChangeCardRepsCount.bind(this, TopItems, item)} />
                                                                </div>
                                                    ...
                                                    {(item.flag == "video") &&
                                                        <CRow>
                                                            ...
                                                            <CCol lg={7}>
                                                                <div style={{ float: 'right' }}>
                                                                    <input id="myInput" type="file" ref={(ref) => this.myInput102 = ref} style={{ display: 'none' }} onChange={this.onChangeCardImageFile.bind(this, TopItems, item)} />
                                                                    ...
                                                                    <div style={{ display: 'inline-block' }}>
                                                                        <CInput className="form-control" id="bcc2" type="email" placeholder="thumbnail" style={{ width: "280px" }} value={item.thumbnailFileName} onClick={(e) => this.myInput102.click()} />

                                                                        <br />  <CInput className="form-control" id="bcc2" type="email" placeholder="Browse Files" style={{ width: "280px" }} value={item.fileName} onClick={(e) => this.myInput100.click()} />
                                                                    </div>
                                                                    <div style={{ display: 'inline-block', marginLeft: '15px' }}>
                                                                        <CButton type="button" color="info" onClick={(e) => this.myInput102.click()}>Upload Image</CButton>
                                                                        <br /><br /> <CButton type="button" color="info" onClick={(e) => this.myInput100.click()}>Upload Video</CButton>
                                                                    </div>
                                    ...
                                    <CListGroupItem  >
                                        <CButton type="button" color="info" variant="ghost" onClick={this.onAddCircuitSubExercise.bind(this, TopItems, "exercise")}> +Add Exercise Card</CButton>
                                        <CButton type="button" color="info" variant="ghost" style={{ marginLeft: '25px' }} onClick={this.onAddCircuitSubExercise.bind(this, TopItems, "note")}> +Add Note Card</CButton>
                                        <CButton type="button" color="info" variant="ghost" style={{ marginLeft: '25px' }} onClick={this.onAddCircuitSubExercise.bind(this, TopItems, "rest")}> +Add Rest Card</CButton>
                                        <CButton type="button" color="info" variant="ghost" style={{ marginLeft: '25px' }} onClick={this.onAddCircuitSubExercise.bind(this, TopItems, "video")}> +Add Video Card</CButton>
                                    </CListGroupItem>
                                </CListGroup> <br />
                            </div>
                        )
                                                    ...
                                                    <CRow>
                                                        ...
                                                        <CCol lg={8} >
                                                            <div style={{ float: 'right' }}>
                                                                ...
                                                                <div style={{ display: 'inline-block', marginLeft: '15px' }}>Sets</div>
                                                                <div style={{ display: 'inline-block', marginLeft: '15px' }}>
                                                                    <CInput type="email" id="date-input" name="date-input" placeholder="3" style={{ width: "70px" }} value={item.Sets}
                                                                        onChange={this.onChangeDividesCardSets.bind(this, item)} />
                                                                </div>

                                                                <div style={{ display: 'inline-block', marginLeft: '15px' }}> {item.Reps}</div>
                                                                <div style={{ display: 'inline-block', marginLeft: '15px' }}>
                                                                    <CInput type="email" id="date-input" name="date-input" placeholder="10" style={{ width: "70px" }} value={item.RepsCount}
                                                ...
                                                <CListGroupItem   >
                                                    <CRow>
                                                        ...
                                                        <CCol lg={7}>
                                                            <div style={{ float: 'right' }}>
                                                                {/* <div style={{ display: 'inline-block' }}> */}
                                                                <input id="myInput" type="file" ref={(ref) => this.myInput202 = ref} style={{ display: 'none' }} onChange={this.onChangeDividesCardImageFile.bind(this, item)} />
                                                                ...
                                                                <div style={{ display: 'inline-block' }}>
                                                                    <CInput className="form-control" id="bcc3" type="email" placeholder="thumnail" style={{ width: "280px" }} value={item.thumbnailFileName} onClick={(e) => this.myInput202.click()} />
                                                                    <br /><CInput className="form-control" id="bcc2" type="email" placeholder="Browse Files" style={{ width: "280px" }} value={item.fileName} onClick={(e) => this.myInput200.click()} />
                                                                </div>
                                                                <div style={{ display: 'inline-block', marginLeft: '15px' }}>
                                                                    <CButton type="button" color="info" onClick={(e) => this.myInput202.click()}>Upload Image</CButton>
                                                                    <br /><br />  <CButton type="button" color="info" onClick={(e) => this.myInput200.click()}>Upload Video</CButton>
                                                                </div>
                                                                {/* </div> */}
                                                                <div style={{ display: 'inline-block', marginLeft: '15px', textAlign: 'center' }}>
                                                                    Auto Play Next Card  </div>
